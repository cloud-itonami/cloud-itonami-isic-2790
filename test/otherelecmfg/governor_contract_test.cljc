(ns otherelecmfg.governor-contract-test
  ...
  operations... does NOT self-issue an electrical-safety certification
  mark') implemented faithfully. The single invariant under test:

    OtherElecEquipAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Other Electrical Equipment
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :welding-equipment}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :welding-equipment (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest invalid-insulation-resistance-mohm-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:insulation-resistance-mohm 999999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-insulation-resistance-mohm} (-> (store/ledger db) last :basis)))
    (is (not= 999999999.0 (:insulation-resistance-mohm (store/batch db "batch-001"))) "fabricated insulation-resistance-mohm never lands in the SSoT")))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "assembler-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
