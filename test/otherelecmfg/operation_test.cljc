(ns otherelecmfg.operation-test
  "Smoke tests for the compiled OtherElecEquipOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `otherelecmfg.governor-contract-test`; the Store contract in
  `otherelecmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "OtherElecEquipOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "assembler-001" :severity :moderate :description "output-current test anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
