(ns otherelecmfg.registry
  "Pure-function domain logic for the ISIC 2790 (other electrical
  equipment) plant-operations coordination actor -- equipment/batch
  verification, shipment-quantity recompute, product-type validation,
  insulation-resistance plausibility validation, defect-rate
  ...
  pre-existing `kotoba-lang/otherelecmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `otherelecmfg.governor` -- the same 'ground truth, not self-report'
  ...
  of issuing an electrical-safety certification mark (this actor NEVER
  does any of those -- see README `What this actor does NOT do`).

  SCOPE: ISIC 2790 is the residual class for manufacture of other
  electrical equipment not elsewhere classified -- electric welding
  and soldering equipment, non-electric domestic-appliance parts, and
  resistors/capacitors n.e.c. -- assembly and test lines (including
  insulation-resistance/megohmmeter withstand testing) producing
  finished units of these product families. This actor coordinates
  ...
  issues electrical-safety compliance marks (e.g. UL/CE/IEC).")

;; ----------------------------- constants -----------------------------

(def valid-product-types
  "The closed set of product-type values a production-batch record may
  declare -- ISIC 2790's residual-class product families. Anything
  else is a fabricated/unrecognized product type -- the governor
  HARD-holds rather than let an invented type pass through."
  #{:welding-equipment :soldering-equipment :appliance-part
    :resistor :capacitor})

(def insulation-resistance-mohm-min
  "Physical floor for a batch's own insulation-resistance (megohmmeter)
  test reading, in MΩ. A resistance reading is never negative."
  0.0)

(def insulation-resistance-mohm-max
  "Physical ceiling for a batch's own insulation-resistance test
  reading, in MΩ. Grounded in the working range of standard production
  megohmmeters used for electrical-equipment QC (e.g. Fluke 1550C/1587
  class insulation testers top out around 200 GΩ = 200,000 MΩ at their
  highest test voltage) -- a reading above this is implausible
  sensor/QC data, not a real routine insulation-resistance test on any
  standard class of other electrical equipment."
  200000.0)
...
(defn batch-verified?
  ...
  (i.e. its product-type/insulation-resistance-mohm/quantity/
...
(defn insulation-resistance-mohm-valid?
  "Is `mohm` a physically plausible insulation-resistance (megohmmeter)
  test reading, in MΩ? Rejects nil, non-numbers, negative values, and
  values beyond `insulation-resistance-mohm-max` -- a fabricated or
  sensor-error reading, never let through as a real test-result fact."
  [mohm]
  (and (number? mohm)
       (>= (double mohm) insulation-resistance-mohm-min)
       (<= (double mohm) insulation-resistance-mohm-max)))
...
(defn register-maintenance
  ...
  the RECORD a plant coordinator would keep. `otherelecmfg.governor`
...
(defn register-shipment
  ...
  outbound electrical-equipment shipment against a verified, registered
  ...
  `otherelecmfg.governor` independently re-verifies the shipment's own
