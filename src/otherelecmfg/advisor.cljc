(ns otherelecmfg.advisor
  "OtherElecEquipAdvisor -- the *contained intelligence node* for the
  ISIC 2790 (other electrical equipment: electric welding/soldering
  equipment, non-electric domestic-appliance parts, resistors/
  capacitors n.e.c.) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/insulation-
  resistance-mohm/quantity/defect-rate), drafts an assembly/test-
  ...
  electrical-equipment shipment coordination proposal against a
  ...
  actuation, freight dispatch, or electrical-safety certification --
  see README `What this actor does NOT do`. Every output is censored
  downstream by `otherelecmfg.governor` before anything touches the
  SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `otherelecmfg.governor`)
  ...
  `otherelecmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [otherelecmfg.registry :as registry]
...
(defn- log-production-batch
  ...
  insulation-resistance-mohm, quantity, defect-rate or verification
...
(defn- schedule-maintenance
  ...
  `otherelecmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft an insulation-failure/electrical-safety/output-current-test-
  hazard concern. ALWAYS `:stake :coordination/safety-concern` -- a
  ...
  technicality). See `otherelecmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `otherelecmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound electrical-equipment shipment coordination
  ...
  `otherelecmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは電気機器その他(溶接・はんだ付け機器、非電気式家庭用機器部品、"
       "抵抗器・コンデンサ等)製造工場プラント運用コーディネーターの助言者です。"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `otherelecmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :otherelecmfg-advisor-proposal
   :op         (:op request)
