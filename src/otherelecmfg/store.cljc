(ns otherelecmfg.store
  "SSoT for the ISIC 2790 (other electrical equipment) plant-operations
  ...
  self-contained (no external other-electrical-equipment-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/insulation-resistance-mohm/
    ...
    - `maintenance`       -- a scheduled assembly/test-bench
                             ...
                             of equipment (`otherelecmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound electrical-equipment
                             shipment DRAFT (`otherelecmfg.registry`'s
                             `register-shipment`).
  ...
  coordinator needs."
  (:require [otherelecmfg.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (otherelecmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (otherelecmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :welding-equipment
                :model "WLD-200A" :insulation-resistance-mohm 500.0
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :capacitor
                :model "CAP-450V-100UF" :insulation-resistance-mohm 1000.0
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :resistor
                :model "RES-10K-1W" :insulation-resistance-mohm 50.0
...
(defn- sample-equipment []
  {"assembler-001" {:id "assembler-001" :kind :assembly-station
                     :verified? true :registered? true
                     :last-maintenance-date "2026-05-01"}
   "test-bench-002" {:id "test-bench-002" :kind :component-test-bench
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `otherelecmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `otherelecmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  assembly-station unit (schedulable for maintenance), one UNVERIFIED/
  unregistered component-test-bench unit (blocks any maintenance
