(ns otherelecmfg.governor
  "Other Electrical Equipment Plant Operations Governor -- the
  independent compliance layer that earns the OtherElecEquipAdvisor
  ...
  secretly tries to self-issue an electrical-safety certification mark
  ...
  `:other-electrical-equipment-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:assembler/actuate`
...
(defn- invalid-insulation-resistance-mohm-violations
  "For `:log-production-batch`, if the patch declares an
  `:insulation-resistance-mohm` that is not a physically plausible
  megohmmeter reading, reject rather than let fabricated/sensor-error
  data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [mohm (:insulation-resistance-mohm (:value proposal))]
      (when (and (some? mohm) (not (registry/insulation-resistance-mohm-valid? mohm)))
        [{:rule :invalid-insulation-resistance-mohm
          :detail (str mohm "MΩ は物理的に妥当な絶縁抵抗試験値の範囲外")}]))))
...
(defn check
  "Censors an OtherElecEquipAdvisor proposal against the governor
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-insulation-resistance-mohm-violations request proposal)
