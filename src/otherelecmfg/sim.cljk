(ns otherelecmfg.sim
  ...
  proposal that tries to self-issue an electrical-safety certification
  ...
  insulation-resistance-mohm reading, and a production-batch patch
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on assembler-001 (verified, registered, assembly station -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on assembler-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-3 on assembler-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible insulation-resistance-mohm -> HARD hold ==")
